...
The approved gateway release is package `0.5.40-9trip.3` on upstream `0.5.40`
with routing contract `session-affinity/v2`. Build its archive with
`npm run cli:pack`; it produces `9router-0.5.40-9trip.3.tgz` at the repository
...
- `npm run release:install` extracts and validates that exact archive into
  `$HOME/.local/share/9router/releases/0.5.40-9trip.3` without overwriting an
  existing release.
