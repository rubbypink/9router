...
- Manual-QA channel: an ephemeral local fake-upstream driver must issue real gateway requests and assert the captured outbound attempts/order without credentials. Routing work uses `tests/manual/proactive-routing-smoke.mjs`; Gemini-continuity work uses `tests/manual/gemini-signature-continuity-smoke.mjs`. Each runs as `node tests/manual/driver-name.mjs --scenario scenario-name --evidence .omo/evidence/scenario-name.json` and removes all child processes/temp data in `finally`.
...
- [ ] 5. Add durable, bounded Gemini signature-continuity storage
  ...
  QA scenarios (name the exact tool + invocation): Happy: `node tests/manual/gemini-signature-continuity-smoke.mjs --scenario gemini-signature-restart-recovery --evidence .omo/evidence/task-5-proactive-quota-combo-provider-fallback.json` stores a synthetic opaque test token, reopens the local DB, and restores only the matching record. Failure: `node tests/manual/gemini-signature-continuity-smoke.mjs --scenario gemini-signature-cross-session-denied --evidence .omo/evidence/task-5-proactive-quota-combo-provider-fallback.json` returns no token and reports a typed continuity miss. Run focused DB tests from `tests/`.
...
- [ ] 6. Preserve Gemini signed parts losslessly and gate incompatible fallback
  ...
  QA scenarios (name the exact tool + invocation): Happy: `node tests/manual/gemini-signature-continuity-smoke.mjs --scenario gemini-exact-signed-tool-continuation --evidence .omo/evidence/task-6-proactive-quota-combo-provider-fallback.json` captures the exact synthetic token only on the matching second request. Failure: `node tests/manual/gemini-signature-continuity-smoke.mjs --scenario gemini-signature-missing-terminal --evidence .omo/evidence/task-6-proactive-quota-combo-provider-fallback.json` captures no fallback endpoint attempt and a `gemini_thought_signature_missing` response. Run focused translator/native-endpoint Vitest files from `tests/`.
